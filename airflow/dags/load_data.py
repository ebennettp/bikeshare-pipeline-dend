...
from operators.s3_to_redshift import S3ToRedshift
from operators.weather_to_redshift import WeatherToRedshift
from operators.load_table import LoadTableOperator
from operators.data_check import DataQualityOperator
...
run_quality_checks = DataQualityOperator(
    ...
    conn_id='redshift',
    cases = [
        {'sql' : 'SELECT COUNT(ride_id) FROM f_trips WHERE ride_id IS NULL', 'answer': 0},
        {'sql' : 'SELECT COUNT(start_time) FROM f_trips WHERE start_time IS NULL', 'answer': 0}
    ]
)
...
joint_operator >> [load_trips_table,
load_weather_table,
load_stations_table,
load_time_table]
